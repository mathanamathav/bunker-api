...
from app.exceptions import AttendanceUpdateInProcessException, InvalidUsernameOrPasswordException, \
    ScrappingError, NoTimeTableDataException, NoSemResultsAvailable
from app.models import AttendanceModel, TimeTableModel
...
class AttendanceWebScrapper:
    ...
    def grade_score(grade: str) -> int:
        grades = {"O": 10, "A+": 9, "A": 8, "B+": 7, "B": 6, "C+": 5, "C": 4, "W": 0, "RA": 0, "SA": 0}
        return grades.get(grade, 0)

    @staticmethod
    def generate_login_request_body(soup: BeautifulSoup, user_name: str, password: str) -> dict:
    ...
    def fetch_time_table(self) -> List[TimeTableModel]:
        ...
        return AttendanceWebScrapper.parse_table_data_as_timetable_models(self.parse_table(table))
